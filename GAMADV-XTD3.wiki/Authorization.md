...
If you are going to create a project from a Google Cloud Platform, verify the following steps:
...
```
[*]  8)  Cloud Resource Manager API v3
```
...
* `projectname <ProjectName>` - Google project name
...
### Basic
Use an existing project with default values for the service account.
```
...
### Advanced
Use an existing project with user-specified values for the service account.
```
...
```
* `<EmailAddress>` - A Google Workspace admin/GCP project manager; if omitted, you will be prompted for the address
* `scopes <APIScopeURLList>` - A set of specific scopes rather than the; if omitted, you will be prompted to select your desired scopes.
...
For `print|show svcaccts`, you can eliminate the password requirement by enabling the following scope in `gam update serviceaccount`;
GAM will then use Service Account access to display Service Accounts.
```
[*]  8)  Cloud Resource Manager API v1
```
...
gam rotate sakey retain_existing [(algorithm KEY_ALG_RSA_1024|KEY_ALG_RSA_2048)|(localkeysize 1024|2048|4096)]
```
...
repeat
  gam rotate sakeys retain_existing
...
gam rotate sakey replace_current [(algorithm KEY_ALG_RSA_1024|KEY_ALG_RSA_2048)|(localkeysize 1024|2048|4096)]
```
...
gam rotate sakeys retain_none [(algorithm KEY_ALG_RSA_1024|KEY_ALG_RSA_2048)|(localkeysize 1024|2048|4096)]
```
...
```
{"file_version": 2, "credentials": {"gam": ""}}
```
