...
- [Draft Import Insert messages](#draft-import-insert-messages)
...
To select messages with a label containing `(` or `{`, you have to replace these characters with `-` in the query.
You can also replace ` `, `/`, `)` and `}` with `-` but it doesn't seem to be required.

* `query "label:Foo -Bar-"` - Select messages with label `Foo (Bar)`

## Draft Import Insert messages
...
For `import message` and `insert message`:
...
Each message can have a text part and/or an HTML part. The message parts can be read from the command line or from a file.

The arguments `deleted`, `nevermarkspam` and `processforcalendar` all default to False.
...
* `showattachments` - Display attachment filename and MIME type
...
By default, message attachment information is not displayed.
* `showattachments` - Display message attachment content for MIME type text/plain, display filename and MIME type for other MIME types
* `attachmentnamepattern <RegularExpression>` - Limit the attachments shown to those whose names match `<RegularExpression>`
