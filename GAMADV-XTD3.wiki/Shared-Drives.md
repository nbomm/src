...
<SharedDriveACLRole> ::= commenter|contentmanager|fileorganizer|manager|organizer|reader|writer
...
* `gam <UserTypeEntity> action ...` - The user named in `<UserTypeEntty>` is used, access in limited to drives for which they are an organizer
...
```
gam redirect csv ./SharedDriveNamesIDs.csv multiprocess [todrive <ToDriveAttribute>*] csv SharedDriveNames.csv gam create teamdrive "~name" csv  [other options as desired]
gam redirect csv ./SharedDriveNamesIDs.csv multiprocess [todrive <ToDriveAttribute>*] csv SharedDriveNames.csv gam user admin@domain.com create teamdrive "~name" asadmin csv  [other options as desired]
```
...
gam [<UserTypeEntity>] show teamdriveinfo <SharedDriveEntity>
        [adminaccess|asadmin]
        [fields <SharedDriveFieldNameList>] [formatjson]
gam [<UserTypeEntity>] show teamdrives [teamdriveadminquery|query <QueryTeamDrive>]
        [adminaccess|asadmin]
        [matchname <RegularExpression>]
        [fields <SharedDriveFieldNameList>] [formatjson]
```
...
### For all Shared Drives
Include a permission match `pm domain domain.com em` for each `domain.com` that is an internal domain.
```
One row per Shared Drive, all ACLs on the same row
gam redirect csv ./SharedDriveACLsAllExternal.csv print teamdriveacls fields id,domain,emailaddress,role,type,deleted pm domain domain.com em pmm or pma skip pmselect

A row per Shared Drive/ACL combination
gam redirect csv ./SharedDriveACLsAllExternalOIPR.csv print teamdriveacls fields id,domain,emailaddress,role,type,deleted pm domain domain.com em pmm or pma skip pmselect oneitemperrow
```
### For selected Shared Drives
Create a CSV file TeamDrives.csv with at least two columns (id, name) for the selected Shared Drives.
...
```
One row per Shared Drive, all ACLs on the same row
gam redirect csv ./SharedDriveACLsAllExternal.csv multiprocess csv ./SharedDrives.csv gam print drivefileacls "~id" addtitle "~name" fields id,domain,emailaddress,role,type,deleted pm domain domain.com em pmm or pma skip pmselect

A row per Shared Drive/ACL combination
gam redirect csv ./SharedDriveACLsAllExternalOIPR.csv multiprocess csv ./SharedDrives.csv gam print drivefileacls "~id" addtitle "~name" fields id,domain,emailaddress,role,type,deleted pm domain domain.com em pmm or pma skip pmselect oneitemperrow
```
...
### Get all top-level files and folders from the Shared Drives
Sustitute an appropriate value for `admin@domain.com`.
```
...
### Purge all top-level files and folders from the Shared Drives
